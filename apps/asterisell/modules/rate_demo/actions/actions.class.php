 ...
 */
class rate_demoActions extends autorate_demoActions
{
  ...
  public function updatePhpRateWithFormFields(PhpRate $phpRate) {
    ...
    $dstChannel = $this->getRequestParameter('dst_channel');
    $calledTelephoneNumber = $this->getRequestParameter('called_telephone_number');
    $cost = $this->getRequestParameter('cost');

    $phpRate->dstChannelPattern = $dstChannel;
    $phpRate->calledTelephoneNumber = $calledTelephoneNumber;

    if (is_numeric($cost)) {
      $phpRate->cost = $cost; 
    } else {
      $allOk = FALSE;
      $this->setErrorMessage("Cost field must be a numeric value");
    }
  ...
  public function handleErrorSave() {
    $this->redirect('rate_demo/edit?id=' . VariableFrame::$arRate->getId());
  }
