...
abstract class CustomCDRProcessing extends PhpRate {
  ...
  protected function linkToArAsteriskAccount($cdr, $accountcode) {
    if (is_null($accountcode) || strlen(trim($accountcode)) == 0) {
      // signal the problem and exit
      // NOTE: I'm testing it in this place, otherwise the cache is confused...
      //
      $p = new ArProblem();
      $p->setDuplicationKey("unknown ArAsteriskAccount NULL");
      $p->setCreatedAt(date("c"));
      $p->setDescription("Asterisk account code can not be empty (NULL or \"\") in CDR with id \"" . $cdr->getId() . "\".");
      $p->setEffect("All these CDRs with empty Account Code will not rated.");
      $p->setProposedSolution("Change the configuration of Asterisk VoIP server.");
      throw (new ArProblemException($p));
    }
